...
import dummyProducts,{ useFetchProduct, useDeleteHandler, IProductType } from "../data/apicalls";
import ProductForm from "./ProductForm";


const ProductList = (): JSX.Element => {
    ReactModal.setAppElement("#root");
    const { data: products, error, isError, isLoading } = useFetchProduct();
    const [showForm, setShowForm] = useState(false);
    const deleteProduct = useDeleteHandler(); 
    const queryClient = useQueryClient();
  
    const mutation = useMutation((id: number) => deleteProduct(id), {
      onSuccess: () => {
